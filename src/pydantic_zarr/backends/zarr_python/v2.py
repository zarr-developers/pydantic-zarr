...
from pydantic_zarr.v2 import ArraySpec, BaseAttr, BaseItem, GroupSpec

from .versions import _zarr_python_version

zarr_python_version = _zarr_python_version()
if zarr_python_version is not None:
    import zarr
    ...
    async def create_spec(node: zarr.Group | zarr.Array) -> GroupSpec[Any, Any] | ArraySpec[Any]:
        """
        Create a GroupSpec or ArraySpec from a stored zarr group or array.
        """
        ...
        if isinstance(node, zarr.Array):
            ...
            raise ValueError(f"node must be a zarr.Array or zarr.Group, got {type(node)}")
    ...
    async def write(
        ...
        **kwargs : Any
        ...
        elif contains_array(StorePath(store=store, path=path), zarr_format=2) and not overwrite:
            msg = (
    ...
    def create_groupspec(group: zarr.Group, *, depth: int = -1) -> GroupSpec[BaseAttr, BaseItem]:
        ...
        for name, item in group.members():
            if isinstance(item, zarr.Array):
                ...
                # convert to dict before the final typed GroupSpec construction
                item_out = create_groupspec(item)
            else:
                msg = (
    ...
    async def persist_groupspec(
        self, store: Store, path: str, *, overwrite: bool = False, **kwargs: object
    ) -> object:
        """
        ...
        **kwargs : Any
        ...
        if await contains_group(StorePath(store, path), zarr_format=2):
        ...
        elif contains_array(StorePath(store, path), zarr_format=2) and not overwrite:
            msg = (
                f"An array already exists at path {path}. "
                "Call to_zarr with overwrite=True to overwrite the array."
            )
            ...
            result = create_group(store=store, overwrite=overwrite, path=path)
    ...
    async def write_arrayspec(
        ...
        **kwargs : Any
        ...
        if await contains_array(StorePath(store, path)):
